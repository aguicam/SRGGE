...
//shader ID
enum {VIEW4,WALL,FLOOR};

Viewer4::Viewer4(const QGLFormat &glf, QWidget *parent) : HW(glf, parent) , mesh_importer(this)
{
    // init vars
    setFocusPolicy(Qt::StrongFocus);
}

void Viewer4::initializeGL()
{
    ...
    //Loading the simplified models as well
...
//    //ANNO
    ...
    hysteresis.push_back(-1);
      ...
      diag.clear();
      //We calculate the diagonals for each model
...
void Viewer4::resizeGL(int w, int h)
{
...
void Viewer4::paintGL()
{
    ...
    // use glsl program
    glUseProgram(m_program[VIEW4].programId());
    ...
    for(int l=0;l<(int)objects.size();l++){// We run all the objects list, and translate and draw the walls
        if(objects[l].type==3){
    ...
    glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_PROJ], 1, GL_FALSE, projection.data());
    glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_MODEL], 1, GL_FALSE, model.data());
    glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_VIEW], 1, GL_FALSE, view.data());
    glUniformMatrix3fv(const_uniform[VIEW4][CONST_UNIFORM_NORMAL], 1, GL_FALSE, normal.data());
    glUniform1i(glGetUniformLocation(m_program[VIEW4].programId(), "colored"), (int)colored);
    /***Computation of Session 4 ***/
    if(!max_throughput){
        float low_cost=INT_MAX;
        int l_pos=0;
        for(int i=0;i<(int)objects.size();i++){
            if(objects[i].lod!=4&&objects[i].type<3&&(i!=hysteresis[0]&&i!=hysteresis[1]&&i!=hysteresis[2]&&i!=hysteresis[3]&&i!=hysteresis[4])){
        ...
        int h_pos=0;
        for(int i=0;i<(int)objects.size();i++){
            if(objects[i].lod!=0&&objects[i].type<3&&(i!=hysteresis[0]&&i!=hysteresis[1]&&i!=hysteresis[2]&&i!=hysteresis[3]&&i!=hysteresis[4])){
                Eigen::Vector4f view_c = view*(Eigen::Vector4f (objects[i].x,0,objects[i].z,1));
                float D = sqrt(pow(view_c[0],2)+pow(view_c[1],2)+pow(view_c[2],2));
                float cost_diff= pow(2,objects[i].lod+1)*D/diag[objects[i].type]-pow(2,objects[i].lod)*D/diag[objects[i].type];
...
/*********************/
    //We draw the Armadillo, bunny and Anno
    draw_Armadillo(model,view);
    draw_Bunny(model,view);
    draw_Anno(model,view);
    ...
    if(fps<22){max_throughput=true;}
    if(fps>=22){max_throughput=false;}


}

QGroupBox* Viewer4::controlPanel()
{
    // init group box
...
void Viewer4::initShaders()
{
    // init uniforms hash table based on uniform variable names and location IDs
    UniformMap uniforms;

    // compile shader, bind attribute vars, link shader, and initialize uniform var table
    initShader(VIEW4, QString(":/SRGGE/Shaders/s4s5s6.vert"), QString(":/SRGGE/Shaders/s4s5s6.frag"), uniforms);
...
void Viewer4::initVertexBuffer()
{ //We initialize all the needed buffers
...
void Viewer4::showFramerate(QGroupBox *groupBox){
...
/**DRAW FUNCTIONS***/
void Viewer4::draw_Armadillo(Eigen::Matrix4f model,Eigen::Matrix4f view){
    Eigen::Matrix4f translation;
    /**DRAW ARMADILLO **/
    /*** We draw LOD 0 ***/
    glBindVertexArray = (_glBindVertexArray) QGLWidget::context()->getProcAddress("glBindVertexArray");
    ...
    for(int l=0;l<(int)objects.size();l++){
           // We run all the objects list, and draw the armadillos with current LOD 0.
          // We scale the the armadillo so it fits into a cell,we translate it to its position, and we rotate it acordingly
        if(objects[l].lod==0&&objects[l].type==0){
            ...
            normal = normal.inverse().transpose();
            glUniformMatrix3fv(const_uniform[VIEW4][CONST_UNIFORM_NORMAL], 1, GL_FALSE, normal.data());
            glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_MODEL], 1, GL_FALSE,translation.data());
            glUniform1i(glGetUniformLocation(m_program[VIEW4].programId(), "lod"), 0);
    ...
    for(int l=0;l<(int)objects.size();l++){
        if(objects[l].lod==1&&objects[l].type==0){
            ...
            glUniformMatrix3fv(const_uniform[VIEW4][CONST_UNIFORM_NORMAL], 1, GL_FALSE, normal.data());
            glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_MODEL], 1, GL_FALSE,translation.data());
            glUniform1i(glGetUniformLocation(m_program[VIEW4].programId(), "lod"), 1);
    ...
    for(int l=0;l<(int)objects.size();l++){
        ...
        if(objects[l].lod==2&&objects[l].type==0){
            ...
            glUniformMatrix3fv(const_uniform[VIEW4][CONST_UNIFORM_NORMAL], 1, GL_FALSE, normal.data());
            glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_MODEL], 1, GL_FALSE,translation.data());
            glUniform1i(glGetUniformLocation(m_program[VIEW4].programId(), "lod"), 2);
    ...
    for(int l=0;l<(int)objects.size();l++){
        ...
        if(objects[l].lod==3&&objects[l].type==0){
            ...
            glUniformMatrix3fv(const_uniform[VIEW4][CONST_UNIFORM_NORMAL], 1, GL_FALSE, normal.data());
            glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_MODEL], 1, GL_FALSE,translation.data());
            glUniform1i(glGetUniformLocation(m_program[VIEW4].programId(), "lod"), 3);
    ...
    for(int l=0;l<(int)objects.size();l++){
        ...
        if(objects[l].lod==4&&objects[l].type==0){
            ...
            glUniformMatrix3fv(const_uniform[VIEW4][CONST_UNIFORM_NORMAL], 1, GL_FALSE, normal.data());
            glUniformMatrix4fv(const_uniform[VIEW4][CONST_UNIFORM_MODEL], 1, GL_FALSE,translation.data());
            glUniform1i(glGetUniformLocation(m_program[VIEW4].programId(), "lod"), 4);
...
void Viewer4::draw_Bunny(Eigen::Matrix4f model,Eigen::Matrix4f view){
    Eigen::Matrix4f translation;
    /**DRAW Bunny 0 **/
    glBindVertexArray = (_glBindVertexArray) QGLWidget::context()->getProcAddress("glBindVertexArray");
    /*** We draw LOD 0 ***/
    ...
    for(int l=0;l<(int)objects.size();l++){
        // We run all the objects list, and draw the bunnies with current LOD 0.
       // We scale the the bunny so it fits into a cell,we translate it to its position, and we rotate it acordingly
...
void Viewer4::draw_Anno(Eigen::Matrix4f model,Eigen::Matrix4f view){
    Eigen::Matrix4f translation;
    /**DRAW ANNO 0 **/
    glBindVertexArray = (_glBindVertexArray) QGLWidget::context()->getProcAddress("glBindVertexArray");
    /*** We draw LOD 0 ***/
    ...
    for(int l=0;l<(int)objects.size();l++){
        // We run all the objects list, and draw the annos with current LOD 0.
       // We scale the the anno so it fits into a cell,we translate it to its position, and we rotate it acordingly
...
void Viewer4::implementColored(QGroupBox*groupBox){
...
void Viewer4::changeColored(int c){
    colored = c;
    update();
}
...
void Viewer4::loadPositions(int type){
    if(type==0){
        ...
        iss>>L; // The first number determines how many models will there be

        for(int l=0;l<L;l++){
            std::getline(infile, line);
            std::istringstream iss(line);
            int cell;
            iss>>cell; // we read  the cell ID

            int k =fmod(cell,N);
            int i = (cell-k)/N;

            models m; // we create a model variable, and we store the ID, the positions and what model it is and its LoD
            m.cellID=cell;
            ...
            float rot;
            iss2>>rot; // we read the rotation and store it on m
            m.rotation=rot;
            objects.push_back(m);// we add the model to the objects list
        }

    }// THE SAME IS PERFORMED FOR THE REST OF OF THE MODELS AND WALL
    if(type==1){
...
void Viewer4::initBuffer_original(){
    // We compute the scale parameter  and the translation on the Y axis
    scaleArm =2/std::max(mesh_->max_[0]-mesh_->min_[0],mesh_->max_[2]-mesh_->min_[2]);
    transYArm=scaleArm*(mesh_->max_[1]+mesh_->min_[1])*0.63;
    diag[0]=diag[0]*scaleArm; // We modify the diagonal according to the scale
...
void Viewer4::initBuffer1_original(){
    // We compute the scale parameter  and the translation on the Y axis
    ...
    diag[1]=diag[1]*scaleBun; // We modify the diagonal according to the scale
...
void Viewer4::initBuffer2_original(){
    // We compute the scale parameter  and the translation on the Y axis
    ...
    diag[2]=diag[2]*scaleAnn;// We modify the diagonal according to the scale
...
//We initialize the data vector with the vertex coordinates, normals and texture coordinates
    float s3=1/sqrt(3);
